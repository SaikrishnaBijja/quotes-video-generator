...
def get_screen_shot(url, output, mode, night_mode):
    lang=''
    t=5
    chromedriver=''
    fake_name=''
    ...
    # args = parse_args('')
    fake_verified=''
    tweet = TweetCapture(mode, night_mode)
    tweet.set_lang(lang)
    tweet.set_wait_time(t)
    if len(chromedriver) > 0:
        tweet.set_chromedriver_path(chromedriver)
    tweet.Fake.create(fake_name, fake_content, fake_username, fake_image, fake_verified)
    try:
        filename = run(tweet.screenshot(url, output))
